...
export class AudioEngine extends EventTarget {
  ...
  play(): void {
    ...
    this.tracks.forEach((track, index) => {
      ...
      console.log(`  - Track ${index + 1} "${track.name}": started at offset ${offset.toFixed(3)}s`);
      ...
      source.onended = () => {
        ...
        if (this.isPlaying && track.sourceNode === source) {
          ...
          if (currentTime >= duration - 0.1) {
            console.log(`[AudioEngine] Track ${index + 1} reached end naturally, stopping playback`);
            this.isPlaying = false;
            this.pausedAt = duration;
            this.dispatchEvent(new Event('ended'));
          } else {
            console.log(`[AudioEngine] Track ${index + 1} ended prematurely at ${currentTime.toFixed(3)}s (expected: ${duration.toFixed(3)}s) - ignoring`);
          }
        }
      };
    });
